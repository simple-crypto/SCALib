...
extern "C"{
#include <stdint.h>
void stella_wrapper( uint32_t merge_value,uint32_t nb_bins, double *score_mat_init, uint32_t *key_init, double *rank, double *rank_min, double *rank_max){
	hel_result_t* result = NULL; //will contain the results of either rank estimation or key enumeration
    double *score_mat_init_new[16];
    int key[16];
    for(int i=0;i<16;i++){
        score_mat_init_new[i] = &score_mat_init[i*256];
        key[i] = key_init[i];
    }
	result = hel_execute_rank(merge_value, nb_bins,score_mat_init_new, key);
	...
	two = 2; //used to compute the bound_start and bound_end using two^(integer)
        *rank_min = conv<double>(rank_estim_min);
        *rank_max = conv<double>(rank_estim_max);
        *rank = conv<double>(rank_estim_rounded);
        }
}
#endif
int main(){

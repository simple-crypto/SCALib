...
from utils import sbox, gen_traces, HW
import numpy as np
...
def cpa_HW_example(pts, traces):
    """
    Perform a Correlation Power Attack against after the first Sbox layer in the first round of the AES.

    pts: (n, 16), the 'n' different (known) plaintext.
    traces: (n, ns), the traces, of 'ns' samples, correpsonding to the execution of the plaintext with the unknown fixed key.

    returns: the key candidate maximising the correlation with an Hamming Weight leakage model.
    """
    # First, we create the models. Note that this step can be done in an offline first step.
    models = generate_hw_models(16, 256, traces.shape[1])
    # Next, we create the CPA object, using the XOR intermediate function
    cpa = Cpa(256, Cpa.Xor)
    # Fit the correlation computation
    cpa.fit_u(traces, pts)
    # Compute the correlation for each variable, associated to each key guess and time sample.
...
def main_cpa_example():
    ...
    print(
        f"Generate {n_traces} simulated traces (Hamming weight + Gaussian noise) with noise standard deviation of {std}"
    )

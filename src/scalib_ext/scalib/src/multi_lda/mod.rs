    ...
    pub fn get_matrices(&self) -> Result<Vec<(Array2<f64>, Array2<f64>, Array2<f64>)>> {
        let vec_tuples = (0..self.nv)
            .map(|var| match self.compute_matrices_var(var) {
                Ok(m) => {
                    let mus = self.order_mu_matrix(&m.mus, var);
                    let s_w = m.s_w;
                    let s_b = m.s_b;
                    let n_traces = m.n_traces;
                    Ok(LdaMatrices::to_tuple(LdaMatrices {
                        s_w,
                        s_b,
                        mus,
                        n_traces,

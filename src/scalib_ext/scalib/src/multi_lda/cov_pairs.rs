...
/// Configuration for the scatter accumulator.
...
impl CovPairs {
    ...
    pub fn new(ns: usize, pairs: impl Iterator<Item = (u32, u32)>) -> Result<Self> {
        let (sorted_pairs, pairs_to_new_idx, blocks) = block_pairs(pairs, ns)?;
        Ok(Self {
            sorted_pairs,
            blocks,
            pairs_to_new_idx,
...
/// Sparse scatter accumulator state.
#[derive(Debug, Clone, Serialize, Deserialize)]
pub struct CovAcc {
    /// Number of traces accumulated.
    pub tot_n_traces: u32,
    /// Scatter values, in the same order as CovPairs::sorted_pairs.
    pub scatter: Vec<i64>,
}
...
impl CovAcc {
    ...
    pub fn update(
        ...
        let split_at = pois.blocks[1..].iter().map(|r| r.start);
        let scatter_blocks = multi_split_at_mut(self.scatter.as_mut_slice(), split_at.clone());
        let pair_blocks = multi_split_at(pois.sorted_pairs.as_slice(), split_at);
    ...
    /// scatter_block and pair_block are matching slices of CovAcc::scatter and
    /// CovPairs::sorted_pairs.
